...
for planeid=1:5%numel(visplanes)
    ...
    if visplanes(planeid)
        ...
        %     pts_x{planeid} = reshape(pts_c{planeid}(1,:),size(x));
        %     pts_y{planeid} = reshape(pts_c{planeid}(2,:),size(x));
        %     pts_z{planeid} = reshape(pts_c{planeid}(3,:),size(x));
        %
    end
end
